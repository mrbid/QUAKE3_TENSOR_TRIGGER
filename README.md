...
- The FNN is much more generalised; although I allow users to specify a custom amount of Units in the `train.py` script this is simpily for novelty, the only useful method of using an FNN in computer vision for this purpose is to have one weight per pixel colour channel and then feed that all into one unit which outputs a binary classification (0 = not a target, 1 = a target) what this does is essentially creates an averaged cookie-cutter filter/map, which is what causes the larger range of generalisation. In some respects an FNN can have positive benefits, such as higher locality, but the higher generalisation does come with a higher rate of missfire. This can be mitigated by increasing the `REPEAT_ACTIVATION` definition in the `quakelive_bluebones_autoshoot.c` file to some degree. The takeaway here is that the FNN will target almost any player model having only been trained on aqua bones model, it's just better at the latter.
