...
Wanting to take the project a little further but also knowing that writing all the code in C from scratch was severely limiting my ability to rapidly prototype I took up [Tensorflow Keras](https://keras.io/about/) using Python and really honed in on that success.
...
Here we are now, this repository is the fruits of my labor upto this point.
...
- The CNN is much more accurate at what it's trained to detect but lacks the locality an FNN has, although this is a double-edged sword...
- The FNN is much more generalised; although I allow users to specify a custom amount of Units in the `train.py` script this is simply for novelty, the only useful method of using an FNN in computer vision for this purpose is to have one weight per pixel colour channel and then feed that all into one unit which outputs a binary classification (0 = not a target, 1 = a target) what this does is essentially creates an averaged cookie-cutter filter/map, which is what causes the larger range of generalisation. In some respects, an FNN can have positive benefits, such as higher locality, but the higher generalisation does come with a higher rate of misfire. This can be mitigated by increasing the `REPEAT_ACTIVATION` definition in the `quakelive_bluebones_autoshoot.c` file to some degree. The takeaway here is that the FNN will target almost any player model having only been trained on aqua bones model, it's just better at the latter.
...
Generally, you don't want to multi-thread these solutions much because you don't want to eat into the games processing resources too much, but if you do have some excess CPU cores to spare the way I recommend scaling up the FNN solution is to train one model per Quake3 player model, then execute each trained model at the same time across multiple CPU cores. This way your VRAM to RAM operation which is generally limited to ~60 FPS can grab a frame and the CPU can process multiple trained networks on that frame at the same time _(one per Quake3 player model)_ and return an attack or not to attack signal faster than bobs your uncle.

The SimpleCNN models are still the best as they only use a 3x3 sample window so they are much more accurate, and, they have much less misfire. Crazy huh, so much work to make these 28x28 models which are higher in computational cost, time invested, etc, and yet still less effective.
