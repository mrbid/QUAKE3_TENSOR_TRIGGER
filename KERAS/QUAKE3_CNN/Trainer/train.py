# James William Fletcher - July 2021
# loads data set from bitmaps, mean normalises
# and then trains a CNN.
...
seed(8008135)
project = "aim_model"
training_iterations = 666
filter_resolution = 16 # 32 is in the 99%+, 2 is good for 94%+
batches = 24

tc = len(glob.glob('../../DATASET/target/*.bmp'))     # target sample count/length
ntc = len(glob.glob('../../DATASET/nontarget/*.bmp')) # non-target sample count/length
...
if isdir(project):
    ...
    if isfile(project + "/nontargets_x.npy"):
        ...
        st = time_ns()
        files = glob.glob("../../DATASET/nontarget/*")
        for f in files:
    ...
    if isfile(project + "/targets_x.npy"):
        ...
        st = time_ns()
        files = glob.glob("../../DATASET/target/*")
        for f in files:
            img = keras.preprocessing.image.load_img(f)
            arr = keras.preprocessing.image.img_to_array(img)
            arr = np.array(arr)
