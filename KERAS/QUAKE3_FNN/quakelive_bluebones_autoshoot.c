...
#include "Trainer/aim_model/aim_model_layers.h"

#pragma GCC diagnostic ignored "-Wgnu-folding-constant"
#pragma GCC diagnostic ignored "-Wunused-result"

#define SCAN_DELAY 1000
#define ACTIVATION_SENITIVITY 0.90f
...
// for use with multiple units
// float processModel(const float* input)
// {
//     const int num_units = 3;
//     float o1[num_units] = {0};
//     uint u = 0;
//     for(int i = 0; u < num_units; i++)
//     {
//         if(i == slall)
//         {
...
//         else
//         {
//             o1[u] += input[i] * aim_model_layer0[i]; // weight
//         }
//     }

//     float out = 0;
//     for(int i = 0; i < num_units; i++)
//         out += o1[i] * aim_model_layer1[i];
//     out += aim_model_layer1[num_units];
        ...
        if(enable == 1)
        {
            ...
            if(crosshair == 1)
            {
                ...
                else
                {
                    ...
                    // XDrawPoint(d, twin, gc, x-sw2-1, y-1);
                    // XDrawPoint(d, twin, gc, x-sw2-1, y);
                    // XDrawPoint(d, twin, gc, x-sw2-1, y+1);
            ...
            if(crosshair == 2)
            {
                ...
                else
                {
                    ...
                    // XDrawPoint(d, twin, gc, x-sw2-2, y-1);
                    // XDrawPoint(d, twin, gc, x-sw2-2, y);
                    // XDrawPoint(d, twin, gc, x-sw2-2, y+1);
